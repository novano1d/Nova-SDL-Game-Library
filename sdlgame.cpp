...
void GridGame::pseudo3dRenderTextured(int FOV, double wallheight)
{
    ...
    double ZBuffer[INTERNAL_RENDER_RES_HORIZ]; // store Z distances for sprite rendering (necessary for occlusion)
    if (!textureBuffer)
    ...
    {
       ThreadData* data = new ThreadData{i, &ZBuffer, (double*)pixels};
       SDL_Thread* threadID = SDL_CreateThread(renderStripe, NULL, data);
    }
